...
def main(real_path, TRADES_path, finetuned_TRADES_path, cgan_path):
    ...
    correlation_finetuned = load_and_compute_correlation(finetuned_TRADES_path)
    ...
    sns.kdeplot(correlation_finetuned, shade=True, color="purple", label='Finetuned TRADES')
    sns.kdeplot(correlation_cgan, shade=True, color="red", label='CGAN')

    plt.xlabel('Correlation Coefficient')
    plt.ylabel('Frequency')
    plt.title('Autocorrelation Log Returns Distribution (4-way)')

    plt.legend()
    file_name = f"corr_coef_join_4way.pdf"
