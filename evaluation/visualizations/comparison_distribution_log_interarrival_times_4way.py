...
def main(real_path, TRADES_path, finetuned_TRADES_path, cgan_path):
    ...
    data_list = [
        ...
        {'df': pd.read_csv(finetuned_TRADES_path), 'label': 'Finetuned TRADES', 'color': 'purple'},
    ...
    plt.xscale('log')
    plt.legend()
    plt.title('Inter-arrival Times on a Log-x Scale (4-way)')
    plt.xlabel('Inter-arrival Time (seconds)')
    plt.ylabel('Frequency')
    plt.grid(True, which='both', axis='both', linestyle='--', linewidth=0.5)

    file_name = "interarrival_time_plot_4way.pdf"
