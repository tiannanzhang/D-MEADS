...
def is_influential(row, ticker, company_name):
    ...
    if ticker_lower in title or company_lower in title:
        ...
        if any(
            kw in title
            for kw in ["movers", "stock", "shares", "drops", "falls", "rises"]
        ):
    ...
    if article and article != "nan":
        ...
        if mention_count >= 3:
            material_kw = sum(
                1
                for kw in ["earnings", "revenue", "quarter", "ceo", "sales"]
                if kw in article
            )
            if material_kw >= 2:
...
def process_and_combine(ticker, company_name):
    ...
    print(
        f"  Combined: {len(csv_df)} CSV + {len(json_df_influential)} JSON = {initial_count}"
    )
