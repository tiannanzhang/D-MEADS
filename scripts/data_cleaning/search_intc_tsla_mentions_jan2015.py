...
if intc_mentions:
    ...
    title_only = sum(
        1 for m in intc_mentions if m["title_match"] and not m["article_match"]
    )
    article_only = sum(
        1 for m in intc_mentions if m["article_match"] and not m["title_match"]
    )
    ...
    for i, mention in enumerate(intc_mentions[:10]):
        ...
        print(f"URL: {row['Url']}")
        print(
            f"Match: {'Title' if mention['title_match'] else ''}{' & ' if mention['title_match'] and mention['article_match'] else ''}{'Article' if mention['article_match'] else ''}"
        )
...
if tsla_mentions:
    # Count matches by type
    title_only = sum(
        1 for m in tsla_mentions if m["title_match"] and not m["article_match"]
    )
    article_only = sum(
        1 for m in tsla_mentions if m["article_match"] and not m["title_match"]
    )
