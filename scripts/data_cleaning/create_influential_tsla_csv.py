...
for idx in influential_indices_0based:
    ...
    if any(word in title for word in ['movers', 'morning', 'futures', 'pre-market', 'after-hours']):
        reason = 'Stock price movement tracking'
    elif any(word in title for word in ['china', 'sales weak', 'slump', 'dives', 'drop', 'falls']):
        reason = 'Negative news - China sales/stock decline'
    elif any(word in title for word in ['target', 'analyst', 'price', 'valuation', 'undervalued']):
        reason = 'Analyst commentary/price target'
    elif any(word in title for word in ['loses', 'losses', '0.5m', '500,000', 'production']):
    ...
    elif any(word in title for word in ['earnings', 'financial results', 'release date']):
        reason = 'Earnings announcement'
    elif 'auto show' in title or 'detroit' in title:
        reason = 'Auto show coverage'
    elif any(word in title for word in ['bmw', 'volkswagen', 'supercharger', 'competition']):
...
for date in key_dates:
    date_articles = influential_df[influential_df['Date'].dt.strftime('%Y-%m-%d') == date]
