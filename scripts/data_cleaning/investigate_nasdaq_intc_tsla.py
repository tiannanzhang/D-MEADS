...
for i, chunk in enumerate(
    pd.read_csv(
        csv_path, chunksize=chunk_size, dtype={"Stock_symbol": str}, low_memory=False
    )
):
    ...
    if len(intc_chunk) > 0:
        ...
        jan_2015_mask = (intc_chunk["Date"].dt.year == 2015) & (
            intc_chunk["Date"].dt.month == 1
        )
    ...
    if len(tsla_chunk) > 0:
        ...
        # Check for January 2015
        jan_2015_mask = (tsla_chunk["Date"].dt.year == 2015) & (
            tsla_chunk["Date"].dt.month == 1
        )
...
if intc_jan2015:
    ...
    for idx, row in intc_jan_df.head(10).iterrows():
        ...
        if pd.notna(row.get("Article")):
            article_preview = (
                str(row["Article"])[:300] + "..."
                if len(str(row["Article"])) > 300
                else str(row["Article"])
            )
